...
export class authUseCase {
    ...
    public async login(user: { username: string, password: string }) {
        try {
            const foundUser = await this.userRepository.findOne(user.username);
            if (!foundUser) {
            ...
            if (testPassword === foundUser.password) {
                ...
                this.userRepository.loginStampByUsername(user.username);
    ...
    public async register(user: I_UserDocument): Promise<void> {
        try {
